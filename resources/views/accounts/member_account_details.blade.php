...
<x-app-layout>
    ...
    <div class="p-2 bg-slate-100 sm:rounded-lg m-1 w-full mx-1 shadow-sm">
        <div>
            @if (empty($memberAccountToDetail[0]->member))
            ...
            @else
                <h1 class="font-bold capitalize text-green-700">
                    {{ $memberAccountToDetail[0]->member.'\'s '.$memberAccountToDetail[0]->Name.' for Financial Year '.$memberAccountToDetail[0]->year}}
                </h1>
            @endif
            <table class="table-auto w-full mt-2">
                ...
                @php
                    ...
                    $totalBalance = 0;
                @endphp
                    @forelse ($memberAccountToDetail as $memberAccount)
                    @php
