<x-app-layout>
    ...
    <div class="p-2 bg-slate-100 sm:rounded-lg m-1 w-full mx-1 shadow-sm">
        <div>
            @if (empty($toDetailMembersAccounts[0]->member))
            ...
            @else
                <h1 class="font-bold uppercase text-green-700">
                    {{ 'Member: '. $toDetailMembersAccounts[0]->member }}
                </h1>
            @endif
            <table class="table-auto w-full mt-2">
                ...
                @php
                    ...
                    $totalBalance = 0;
                @endphp
                    @forelse ($toDetailMembersAccounts as $account)
                    @php
                    ...
                    @endphp
                        <tr class="border-b-2 border-gray-300">
                            <td class="py-0 px-6 text-left">
                                ...
                                <a href="{{ route('member.memberAccountDetails', ['member_id'=>$account->member_id, 'account_id'=>$account->account_id]) }}">{{ $account->Name }}</a>
                            </td>
